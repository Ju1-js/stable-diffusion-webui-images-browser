...
tabs_list = [
    ...
    faverate_tab_name,
    "Others",
]  # txt2img-grids and img2img-grids added by HaylockGrant
...
def get_image_page(img_path, page_index, filenames, keyword, sort_by):
    ...
    visible_num = (
        ...
        if idx_frm + num_of_imgs_per_page < length
        else length % num_of_imgs_per_page
    )
...
def change_dir(img_dir, path_recorder, load_switch, img_path_history):
    ...
    if warning is None:
        ...
        if os.path.exists(path_recorder_filename):
            send2trash.send2trash(path_recorder_filename)
        ...
        return (
            ...
            gr.Dropdown.update(choices=path_recorder, value=img_dir),
            path_recorder,
            img_dir,
        )
    else:
...
def create_tab(tabname):
    ...
    if tabname == "txt2img":
        ...
        tabname == "txt2img-grids"
    ):  # added by HaylockGrant to add a new tab for grid images
        dir_name = opts.outdir_txt2img_grids
    elif (
        tabname == "img2img-grids"
    ):  # added by HaylockGrant to add a new tab for grid images
    ...
    with gr.Row(visible=custom_dir):
        img_path = gr.Textbox(
            ...
            placeholder="Input images directory",
            interactive=custom_dir,
        )
        img_path_history = gr.Dropdown(path_recorder)
        path_recorder = gr.State(path_recorder)

    with gr.Row(
        visible=not custom_dir, elem_id=tabname + "_images_history"
    ) as main_panel:
        with gr.Column():
            with gr.Row():
                with gr.Column(scale=2):
                    ...
                    history_gallery = gr.Gallery(
                        show_label=False, elem_id=tabname + "_images_history_gallery"
                    ).style(grid=opts.images_history_page_columns)
                    with gr.Row() as delete_panel:
                        with gr.Column(scale=1):
                            delete_num = gr.Number(
                                value=1, interactive=True, label="delete next"
                            )
                        with gr.Column(scale=3):
                            delete = gr.Button(
                                "Delete", elem_id=tabname + "_images_history_del_button"
                            )

                with gr.Column():
                    with gr.Row():
                        sort_by = gr.Radio(
                            value="date", choices=["path name", "date"], label="sort by"
                        )
                        keyword = gr.Textbox(value="", label="keyword")
                    with gr.Row():
                        with gr.Column():
                            img_file_info = gr.Textbox(
                                label="Generate Info", interactive=False, lines=6
                            )
                            img_file_name = gr.Textbox(
                                value="", label="File Name", interactive=False
                            )
                            img_file_time = gr.HTML()
                    with gr.Row(
                        elem_id=tabname + "_images_history_button_panel"
                    ) as button_panel:
                    ...
                    with gr.Row(visible=False):
                        renew_page = gr.Button(
                            "Renew Page", elem_id=tabname + "_images_history_renew_page"
                        )
                        visible_img_num = gr.Number()
                        tabname_box = gr.Textbox(tabname)
                        image_index = gr.Textbox(value=-1)
                        set_index = gr.Button(
                            "set_index", elem_id=tabname + "_images_history_set_index"
                        )
                        filenames = gr.State([])
                        ...
                        info1 = gr.Textbox()
                        info2 = gr.Textbox()
                        load_switch = gr.Textbox(
                            value="load_switch", label="load_switch"
                        )
    ...
    img_path.submit(
        change_dir,
        inputs=[img_path, path_recorder, load_switch, img_path_history],
        outputs=change_dir_outputs,
    )
    img_path_history.change(
        change_dir,
        inputs=[img_path_history, path_recorder, load_switch, img_path_history],
        outputs=change_dir_outputs,
    )
    ...
    delete.click(
        delete_image,
        inputs=[delete_num, img_file_name, filenames, image_index, visible_img_num],
        outputs=[filenames, delete_num, visible_img_num],
    )
    delete.click(
        fn=None,
        _js="images_history_delete",
        inputs=[delete_num, tabname_box, image_index],
        outputs=None,
    )
    if tabname != faverate_tab_name:
    ...
    turn_page_switch.change(
        fn=None, inputs=[tabname_box], outputs=None, _js="images_history_turnpage"
    )
    turn_page_switch.change(
        fn=lambda: (gr.update(visible=False), gr.update(visible=False)),
        inputs=None,
        outputs=[delete_panel, button_panel],
    )
    ...
    set_index.click(
        ...
        inputs=[tabname_box, image_index, page_index, filenames],
        outputs=[img_file_name, img_file_time, image_index, hidden],
    )
    set_index.click(
    ...
    hidden.change(
        fn=modules.extras.run_pnginfo,
        inputs=[hidden],
        outputs=[info1, img_file_info, info2],
    )

    try:
        modules.generation_parameters_copypaste.bind_buttons(
            send_to_buttons, img_file_name, img_file_info
        )
    except:
        pass
...
def on_ui_tabs():
    ...
    num_of_imgs_per_page = int(
        opts.images_history_page_columns * opts.images_history_page_rows
    )
            ...
            for tab in tabs_list:
                with gr.Tab(tab):
                    with gr.Blocks(analytics_enabled=False):
                        create_tab(tab)
        gr.Checkbox(
            opts.images_history_preload, elem_id="images_history_preload", visible=False
        )
        gr.Textbox(
            ",".join(tabs_list), elem_id="images_history_tabnames_list", visible=False
        )
...
def on_ui_settings():
    ...
    shared.opts.add_option(
        "images_history_preload",
        shared.OptionInfo(False, "Preload images at startup", section=section),
    )
    shared.opts.add_option(
        "images_record_paths",
        shared.OptionInfo(
            True, "Record accessable images directories", section=section
        ),
    )
    shared.opts.add_option(
        "images_delete_message",
        shared.OptionInfo(
            True, "Print image deletion messages to the console", section=section
        ),
    )
    shared.opts.add_option(
        "images_history_page_columns",
        shared.OptionInfo(6, "Number of columns on the page", section=section),
    )
    shared.opts.add_option(
        "images_history_page_rows",
        shared.OptionInfo(6, "Number of rows on the page", section=section),
    )
    shared.opts.add_option(
        "images_history_pages_perload",
        shared.OptionInfo(20, "Minimum number of pages per load", section=section),
    )
